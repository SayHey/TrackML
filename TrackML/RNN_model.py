...
def build_output(lstm_output, in_size, out_size):
    ''' Build an output fully connected layer.
    ...
    with tf.variable_scope('output'):
        ...
        output_w = tf.Variable(tf.truncated_normal(w_shape, stddev=0.1))
        output_b = tf.Variable(tf.zeros(out_size))
    
    # Since output is a bunch of rows of RNN cell outputs, logits will be a
    # bunch
    # of rows of logit outputs, one for each step and sequence
    logits = tf.matmul(x, output_w) + output_b
...
def build_loss(logits, targets):
    ...
    loss = tf.losses.mean_squared_error(logits=logits, labels=targets)
